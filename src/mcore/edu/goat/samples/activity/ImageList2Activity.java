package mcore.edu.goat.samples.activity;
import mcore.edu.goat.samples.activity.ImageList1Activity.Dir;
import java.io.File;
					...
					if (bitmap != null) {
						...
						try {
							if (Build.VERSION.SDK_INT>=Build.VERSION_CODES.HONEYCOMB) {
								if (imageMode) {
									new ImageDownloaderTask(holder.image).executeOnExecutor(AsyncTask.THREAD_POOL_EXECUTOR, FileIoUtil.copyToInnerFile(getApplicationContext(), getItem(position)[Dir.PATH], true));
								}
								else {
								...
								if (imageMode) {
									new ImageDownloaderTask(holder.image).execute( FileIoUtil.copyToInnerFile(getApplicationContext(), getItem(position)[Dir.PATH], true));
								}
								else {
