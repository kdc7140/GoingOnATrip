	...
	private class MyAdapter extends BaseAdapter {
		...
		public View getView(int position, View view, ViewGroup viewGroup) {
			...
			if (bitmap != null) {
				...
				if (imageMode) {
					try {
						new ImageDownloaderTask(holder.picture).execute(FileIoUtil.copyToInnerFile(getApplicationContext(), item.mArray.get(0)[Dir.PATH], true));
					} catch (IOException e) {
